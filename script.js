/* === TETRIS NÉON FUTURISTE - Script complet (boutons modernes + gestes tactiles) === */
...
function drawNextPiece() {
  ...
  shape.forEach(([dx, dy]) => {
    ...
    nextCtx.fillRect(offsetX + dx * block, offsetY + dy * block, block - 2, block - 2);
    nextCtx.strokeStyle = '#ffffff';
    nextCtx.lineWidth = 2;
    nextCtx.strokeRect(offsetX + dx * block, offsetY + dy * block, block - 2, block - 2);
  });
...
function softDrop() {
  ...
  else { score += 1; updateUI(); draw(); }
}

function hardDrop() {
  if (!currentPiece || gameOver || paused) return;
  let dropDistance = 0;
  while (!collision(currentPiece)) { currentPiece.y++; dropDistance++; }
  currentPiece.y--;
...
function flashBoard() {
  ctx.fillStyle = 'rgba(0, 249, 255, 0.32)';
  ctx.fillRect(0, 0, canvas.width, canvas.height);
  setTimeout(() => { if (!gameOver && !paused) draw(); }, 70);
}

function drawGrid() {
  ctx.strokeStyle = 'rgba(0, 249, 255, 0.1)';
  ctx.lineWidth = 1;
...
function playSound(type) {
  ...
  let freq = 220, duration = 0.12, volume = 0.25;
  switch (type) {
    case 'move': freq=180; duration=0.05; volume=0.16; break;
    case 'rotate': freq=410; duration=0.09; volume=0.2; break;
    case 'lock': freq=135; duration=0.16; volume=0.28; break;
    case 'clear': freq=650; duration=0.32; volume=0.32; osc.type='square'; break;
    case 'hard': freq=85; duration=0.22; volume=0.38; break;
  }
...
// === GESTES TACTILES MODERNES (swipe + tap) ===
...
function handleTouchEnd(e) {
  ...
  // Tap court = rotation
  if (duration < 220 && absX < 25 && absY < 25) {
    rotatePiece();
    return;
  }
  ...
  // Swipe vers le bas = soft drop ou hard drop
  if (deltaY > 40) {
    if (duration < 180 && deltaY > 90) hardDrop();
    else softDrop();
  }
}
...
function init() {
  ...
  window.addEventListener('resize', () => {
    ...
    window.resizeTimeout = setTimeout(resizeGame, 140);
  });

  // Clavier
  document.addEventListener('keydown', handleKeyboard);

  // Gestes tactiles modernes sur le canvas
  ...
  console.log('%c[Tetris Neon] Version moderne avec gestes tactiles prête !', 'color:#00f9ff');
}
